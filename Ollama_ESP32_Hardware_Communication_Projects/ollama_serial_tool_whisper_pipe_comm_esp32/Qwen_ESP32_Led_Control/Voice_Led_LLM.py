...
import serial, time
esp = serial.Serial("COM9", 115200, timeout=2)   # <-- YOUR port (see below)
time.sleep(2)                                      # ESP32 auto-resets on open; wait for boot
esp.reset_input_buffer()
esp.write(b"PING\n")
print("link check:", esp.readline().decode().strip())   # must print PONG before you trust anything
